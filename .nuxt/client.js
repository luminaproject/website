...
function mapTransitions(Components, to, from) {
  ...
  return Components.map(Component => {
    ...
    if (from && from.matched.length && from.matched[0].components.default) {
      const from_transitions = componentTransitions(
        from.matched[0].components.default
      )
      Object.keys(from_transitions)
        .filter(
          key =>
            from_transitions[key] && key.toLowerCase().indexOf('leave') !== -1
        )
        .forEach(key => {
...
async function render(to, from, next) {
  ...
  const _next = path => {
    ...
    nextCalled = true
    const matches = []
    _lastPaths = getMatchedComponents(from, matches).map((Component, i) =>
      compile(from.matched[matches[i]].path)(from.params)
    )
    next(path)
  }
  ...
  try {
    ...
    if (!nextCalled) {
      ...
      _lastPaths = Components.map((Component, i) =>
        compile(to.matched[matches[i]].path)(to.params)
      )
      next()
    }
  } catch (error) {
  ...
  Vue.nextTick(() => {
    ...
    instances.forEach((instance, i) => {
      ...
      if (
        ...
        _lastPaths[i] === instance.constructor._path &&
